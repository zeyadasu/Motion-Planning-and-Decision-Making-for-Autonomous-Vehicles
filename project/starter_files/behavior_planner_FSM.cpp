...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance =(0-velocity_mag*velocity_mag)/(2*-_max_accel);  // <- Fix This...... I used the "calc_distance" from velocity_profile_generator
  //I used : (v_f*v_f-v_i*v_i)/(2*a)  where 'a' is negative since it is deceleration and 'v_f; is 0 since i will need to come to a stop. 
